...
export function ReconciliationDashboard({ matches, canManage, universe, sumupRows = [] }: { matches: MatchRow[]; canManage: boolean; universe?: ComparableUniverseReport; sumupRows?: ComparableSumupRow[] }) {
  ...
  const unmatchedSumup = sumupRows.filter((row) => !matchedSumupIds.has(row.id) && ['pos', 'ecom'].includes(String(row.paymentType ?? '').toLowerCase()))
  ...
  const filteredMatches = matches.filter((row) => {
    ...
    if (sourceFilter === 'tiny') return true
  ...
  const comparableMatchRate = universe && universe.TINY_COMPARABLE_COUNT > 0 ? matched.length / universe.TINY_COMPARABLE_COUNT : matchRate
  return <div className="space-y-6">
    <section aria-labelledby="reconciliation-kpis"><div className="mb-3 flex flex-wrap items-end justify-between gap-3"><div><h2 id="reconciliation-kpis" className="text-lg font-semibold">Resumo da reconciliação</h2><p className="text-sm text-muted-foreground">Universo comparável: {universe?.COMPARABLE_START_DATE ?? '—'} a {universe?.COMPARABLE_END_DATE ?? '—'} · sem fuzzy match.</p></div><div className="flex flex-wrap gap-2" aria-label="Fontes de reconciliação"><Badge variant="outline">Tiny · {universe?.TINY_COMPARABLE_COUNT ?? matches.length} comparáveis</Badge><Badge variant="outline">SumUp · {universe?.SUMUP_COMPARABLE_COUNT ?? unmatchedSumup.length} comparáveis</Badge><Badge variant="secondary">PIX e dinheiro · fora do SumUp</Badge></div></div>
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-7"><Metric label="Match %" value={`${(comparableMatchRate * 100).toFixed(1)}%`} /><Metric label="Valor conciliado" value={formatBRL(reconciledValue)} /><Metric label="Matched" value={`${matched.length} · ${formatBRL(reconciledValue)}`} /><Metric label="Unmatched Tiny" value={`${matches.filter((r) => r.status === 'nao_reconciliado').length} · ${formatBRL(Number(matches.filter((r) => r.status === 'nao_reconciliado').reduce((s, r) => s + Number(r.olist_accounts_receivable?.valor ?? 0), 0)) )}`} tone="warning" /><Metric label="Unmatched SumUp" value={`${unmatchedSumup.length} · ${formatBRL(unmatchedSumup.reduce((s, r) => s + r.value, 0))}`} tone="warning" /><Metric label="Ambiguous" value={String(matches.filter((r) => r.status === 'conflito').length)} tone="warning" /><Metric label="Value variance" value={formatBRL(variance)} tone={Math.abs(variance) > 0.01 ? 'warning' : 'default'} /></div>
    </section>
    <section className="grid gap-4 lg:grid-cols-2"><div className="rounded-lg border bg-card p-4"><h2 className="font-semibold">Tiny x SumUp comparável</h2><div className="mt-4 space-y-3"><Bar label="Tiny" value={universe?.TINY_COMPARABLE_VALUE ?? totalTiny} max={Math.max(universe?.TINY_COMPARABLE_VALUE ?? totalTiny, universe?.SUMUP_COMPARABLE_VALUE ?? 0)} /><Bar label="SumUp" value={universe?.SUMUP_COMPARABLE_VALUE ?? candidateValue} max={Math.max(universe?.TINY_COMPARABLE_VALUE ?? totalTiny, universe?.SUMUP_COMPARABLE_VALUE ?? 0)} /></div><p className="mt-3 text-xs text-muted-foreground">A diferença não é convertida em match: transação, período, status e base bruta precisam permanecer rastreáveis.</p></div><div className="rounded-lg border bg-card p-4"><h2 className="font-semibold">Itens que exigem ação</h2><p className="mt-1 text-sm text-muted-foreground">Ordenados por impacto financeiro, com conflito separado de ausência de correspondência.</p><div className="mt-3 flex flex-wrap gap-2"><Badge variant="destructive">Conflitos · {matches.filter((r) => r.status === 'conflito').length}</Badge><Badge variant="warning">Tiny sem par · {matches.filter((r) => r.status === 'nao_reconciliado').length}</Badge><Badge variant="warning">SumUp sem par · {unmatchedSumup.length}</Badge></div></div></section>
    ...
    {exceptions.length > 0 && <section className="rounded-lg border border-amber-300/70 bg-amber-50/50 p-4 dark:bg-amber-950/20"><h2 className="font-semibold">Exceções que exigem decisão</h2><p className="mt-1 text-sm text-muted-foreground">Conflitos e parcelas não reconciliadas aparecem primeiro; nenhuma diferença é incorporada ao caixa silenciosamente.</p><div className="mt-3 space-y-2">{filteredMatches.filter((row) => !resolved.has(row.status)).slice(0, 8).map((row) => <div key={row.id} className="flex flex-wrap items-center justify-between gap-2 rounded-md border bg-background px-3 py-2 text-sm"><span>{row.olist_accounts_receivable?.numero_documento ?? row.olist_accounts_receivable?.historico ?? 'Parcela sem identificação'}</span><span className="text-muted-foreground">{row.status === 'conflito' ? 'Conflito: escolha o candidato correto' : 'Sem correspondência confirmada'}</span><span className="font-mono">{formatBRL(Number(row.olist_accounts_receivable?.valor ?? 0))}</span></div>)}</div></section>}
