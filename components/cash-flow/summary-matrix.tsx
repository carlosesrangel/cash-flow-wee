    ...
    <div className="overflow-x-auto rounded-md border bg-card">
      <table className="min-w-max w-full text-left text-sm">
        ...
        <tbody>
          ...
          {matrix.saldoFinal && <tr className="border-t bg-primary/5 font-semibold"><th className="sticky left-0 bg-card px-4 py-3">SALDO FINAL</th>{matrix.saldoFinal.map((balance, index) => <td key={index} className="px-3 py-3 text-right font-mono">{balance === null ? '?' : formatBRL(balance)}</td>)}<td className="px-4 py-3 text-right font-mono">{matrix.saldoFinal.at(-1) == null ? '?' : formatBRL(matrix.saldoFinal.at(-1)!)}</td></tr>}
        </tbody>
      </table>
    </div>
  )
