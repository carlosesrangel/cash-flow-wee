...
export default async function ImpostosPage() {
  ...
  const [plans, orders] = await Promise.all([
    ...
    fetchAllPages<Order>((start, end) => client.from('olist_orders').select('data, valor_total_pedido, situacao').eq('org_id', member.orgId).range(start, end), 'Falha ao carregar vendas para impostos'),
  ])
  ...
  for (const order of orders) if (order.data && Number(order.valor_total_pedido) > 0 && !['cancelado', 'cancelada', 'cancelled', 'canceled'].includes(String(order.situacao ?? '').toLowerCase())) actualByMonth.set(order.data.slice(0, 7), (actualByMonth.get(order.data.slice(0, 7)) ?? 0) + Number(order.valor_total_pedido))
  ...
  return <div className="space-y-6"><PageHeader title="Impostos" subtitle="Base canônica: pedidos válidos + plano mensal" description="Simples Nacional com fórmula de alíquota efetiva e vencimento no dia 20 do mês seguinte. Sem RBT12, o valor fica zerado e sinalizado." /><div className="grid gap-4 md:grid-cols-3"><MetricCard label="Imposto nos próximos 60 dias" value={formatBRL(upcoming.reduce((sum, item) => sum + item.valorImposto, 0))} accentColor="red" footnote={`${upcoming.length} obrigação(ões)`} /><MetricCard label="Receita projetada (12 meses)" value={formatBRL(schedule.reduce((sum, item) => sum + item.receitaProjetada, 0))} accentColor="navy" /><MetricCard label="Imposto projetado (12 meses)" value={formatBRL(schedule.reduce((sum, item) => sum + item.valorImposto, 0))} accentColor="navy" /></div><Card><CardHeader><CardTitle className="text-lg">Calendário de Obrigações</CardTitle></CardHeader><CardContent><TaxScheduleTable schedule={schedule} today={today} /></CardContent></Card></div>
}
