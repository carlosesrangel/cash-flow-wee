...
import { calculateRFVScore, getRFVSegmentBadge } from '@/lib/analytics/rfv'
...
export default function ClientesPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      />
      <Card><CardContent className="pt-5"><p className="text-sm font-semibold">Como ler o RFV</p><p className="mt-1 text-sm text-muted-foreground">Recência = tempo desde a última compra. Frequência = quantidade de compras válidas. Valor = LTV, a soma acumulada das compras válidas do cliente. Os scores de cada dimensão vão de 1 a 5 e formam os segmentos exibidos.</p></CardContent></Card>
      ...
      {selectedCustomerId && <Card><CardContent className="pt-6">{detailLoading ? <Skeleton className="h-48" /> : customerDetail ? <div className="space-y-4"><div className="flex items-start justify-between"><div><h2 className="text-lg font-semibold">Histórico factual do cliente</h2><p className="text-sm text-muted-foreground">{customerDetail.contact?.email || 'Contato sem e-mail'} · {customerDetail.contact?.telefone || customerDetail.contact?.celular || 'Telefone não informado'}</p></div><button type="button" onClick={() => { setSelectedCustomerId(null); setCustomerDetail(null) }} className="text-sm text-muted-foreground hover:text-foreground">Fechar</button></div><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-6"><MetricCard label="LTV" value={formatBRL(customerDetail.summary.revenue)} /><MetricCard label="Pedidos" value={customerDetail.summary.orders} /><MetricCard label="Ticket médio" value={formatBRL(customerDetail.summary.averageOrderValue)} /><MetricCard label="Primeira compra" value={customerDetail.summary.firstOrderDate ? formatDateOnlyBR(customerDetail.summary.firstOrderDate) : '—'} /><MetricCard label="Última compra" value={customerDetail.summary.lastOrderDate ? formatDateOnlyBR(customerDetail.summary.lastOrderDate) : '—'} /><MetricCard label="Itens" value={customerDetail.summary.units} /></div><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-left text-sm"><thead className="border-b"><tr><th className="py-2">Data</th><th className="py-2">Pedido</th><th className="py-2">Pagamento</th><th className="py-2">Parcelas</th><th className="py-2 text-right">Valor</th><th className="py-2">Status</th><th className="py-2">Itens</th></tr></thead><tbody>{customerDetail.history.map((order: any, index: number) => <tr key={`${order.pedido}-${index}`} className="border-b last:border-0 align-top"><td className="py-2">{order.data ? formatDateOnlyBR(order.data) : '—'}</td><td className="py-2">{order.pedido ?? '—'}</td><td className="py-2">{order.formaPagamento ?? '—'}</td><td className="py-2">{order.parcelas ?? '1/1'}</td><td className="py-2 text-right font-mono">{formatBRL(order.valorTotalPedido)}</td><td className="py-2">{order.status ?? '—'}</td><td className="py-2"><details><summary className="cursor-pointer text-primary">{order.itens.length} produto(s)</summary><ul className="mt-2 space-y-1 text-xs">{order.itens.map((item: any, itemIndex: number) => <li key={`${item.sku ?? item.produto}-${itemIndex}`}>{item.produto ?? 'Produto sem descrição'} · {item.quantidade ?? '—'} × {formatBRL(item.valorUnitario)} = {formatBRL(item.valorTotal)}</li>)}</ul></details></td></tr>)}</tbody></table></div></div> : <p className="text-sm text-muted-foreground">Não foi possível carregar o detalhe.</p>}</CardContent></Card>}
    </div>
  )
}
